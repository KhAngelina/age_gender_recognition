...
class FacialImageProcessing:
    ...
    def __init__(self, print_stat=False, minsize=40):
        self.print_stat = print_stat
        self.minsize = minsize
        #self.sess, self.pnet, self.rnet, self.onet = FacialImageProcessing.load_mtcnn(None)
        if age_gender_one_model:
            self.age_gender_sess, self.age_gender_fun=self.load_age_gender('age_gender_tf2_new-01-0.14-0.92.pb') #age_gender_tf2_192-01-0.16-0.92 age_gender_tf2_new-01-0.14-0.92
            #self.age_gender_sess,self.age_gender_fun=self.load_age_gender('D:/src_code/DNN_models/my_tf/age_gender_tf2_new-01-0.14-0.92.pb') #age_gender_tf2_192-01-0.16-0.92 age_gender_tf2_new-01-0.14-0.92
        else:
            self.gender_sess,self.gender_fun = self.load_gender('D:/src_code/DNN_models/my_tf/gender_mobilenet224_02-0.92.pb')
            #self.age_sess,self.age_fun=self.load_age('D:/src_code/DNN_models/my_tf/age_mobilenet192_tf1_regr.pb')
            self.age_sess,self.age_fun=self.load_age('D:/src_code/DNN_models/my_tf/age_mobilenet192_tf1_softmax-13-0.15.pb')

    def close(self):
        self.sess.close()
        if age_gender_one_model:
            self.age_gender_sess.close()        
        else:
    ...
    def load_age_gender(self,modelpath):
        graph=FacialImageProcessing.load_graph(modelpath)
        ...
        def age_gender_fun(img):
            ...
            facial_features=facial_features[0]/np.sqrt(np.sum(facial_features[0]**2))
        ...
        return sess,age_gender_fun

    def load_gender(self,modelpath):
        graph=FacialImageProcessing.load_graph(modelpath)
        gender_out=graph.get_tensor_by_name('predictions/Sigmoid:0')
        #gender_out=graph.get_tensor_by_name('prob:0')
        print(gender_out)
        print([n.name for n in graph.as_graph_def().node if 'data' in n.name])
        gender_in=graph.get_tensor_by_name('input_1:0')
        #gender_in=graph.get_tensor_by_name('data:0')
        ...
        def gender_fun(img):
            ...
            if self.print_stat:
                print ('gender',preds)
            return preds
        return sess,gender_fun

    def load_age(self,modelpath):
        graph=FacialImageProcessing.load_graph(modelpath)
        #print([n.name for n in graph.as_graph_def().node if 'predictions' in n.name])
        #age_out=graph.get_tensor_by_name('predictions/BiasAdd:0')
        age_out=graph.get_tensor_by_name('predictions/Softmax:0')
        #age_out=graph.get_tensor_by_name('prob:0')
        print(age_out)
        age_in=graph.get_tensor_by_name('input_1:0')
        #age_in=graph.get_tensor_by_name('lambda_1_input:0')
        #age_in=graph.get_tensor_by_name('data:0')
    ...
    def process_image(self,draw):
        ...
        for b in bounding_boxes:
            ...
            if x2>x1 and y2>y1:
                ...
                w,h=x2-x1,y2-y1
                #max_size=max(w,h)
                #orig_x1,orig_x2,orig_y1,orig_y2=x1,x2,y1,y1
                dw,dh=10,10 #max(w//8,10),max(h//8,10) #w//6,h//6
                #dw=dh=5*(ind+1) #w//6,h//6
    ...
    def show_detection_results(self,draw):
        ...
        for i,b in enumerate(bounding_boxes):
            ...
            if is_male:
                ...
                clr=(0, 0, 255)
            #cv2.rectangle(draw, (b[0], b[1]), (b[2], b[3]), clr)
            #cv2.putText(draw,age, (b[0], max(b[1]-10,0)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,0))
            cv2.rectangle(draw, (y1, x1), (y2, x2), clr)
            cv2.putText(draw,age, (y1, x1+10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,0))
    ...
    def load_mtcnn(model_path):
        if not model_path:
            ...
            print("lala" + os.path.realpath(__file__));
            
        graph = FacialImageProcessing.load_graph(os.path.join(model_path,'mtcnn.pb'),'')
        ...
        return sess,pnet_fun, rnet_fun, onet_fun
